...
void printGrid(string name, bool **grid, int n);
...
int main(int argc, string argv[])
{
    ...
    {
        for (int z2 = 0; z2 < candidate_count; z2++)
        {
            printf("%d ", preferences[z][z2]);
        }
        printf("\n");
    }
    return 0;
...
bool is_cycle(int start, int end)
{
    // Check if arrow is pointing to start
    if (end == start)
    {
        return true;
    }

    // Searching for possible path to self
    for (int i = 0; i < candidate_count; i++)
    {
        if (locked[end][i])
        {
